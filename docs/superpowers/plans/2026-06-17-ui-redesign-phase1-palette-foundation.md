...
const SURFACE := Color("#ECE0C4")          # warm hand-painted garden-bed cream (the play stage)
const SURFACE_FRAME := Color("#D8B483")    # warm wood-tan board frame
const CELL_EMPTY := Color("#E6DBBA")       # an empty playable cell (inset on the surface)
...
const NEAR_UNLOCK := Color("#E2D6B6")      # a cell one merge from opening
...
```gdscript
const GROUND := Color("#ECE0C4")        # was olive #3F6B43 — now the warm garden-bed SURFACE
const GROUND_EDGE := Color("#D8B483")   # was #33402F — now SURFACE_FRAME (warm wood-tan)
