...
| `Quests.current_band` (`quests.gd:21`) → `zone_map(quest_zone_for_level(level))` | indexes `QUEST_CLICKS_PER_COIN := [8,7,6,5,4]` — how many quest clicks a coin costs | the band boundaries move with the cadence: L10/17/23/30 (old hand-authored table) → L8/20/38/62 (derived) — the ~2x quest-coin-per-click ramp now arrives 30+ levels later than before | **NOT intentional / not accounted for** — this is a SECOND, independent driver of the measured slowdown: it feeds back into the very CLOCK the gates derive from (quest coins are the clock), and `CLUSTER_LEVEL_LEAD` does not touch it at all |
| `SELL_MAP_BAND` (`grove_data:282`), reached through line availability | the sell-price multiplier a line's items fetch, keyed by `zone_map` of the line's zone | lines 16/17/18/19 (Coral Reef + Cherry Blossom base/special lines) now arrive L38–75 instead of the old L23–34 | **intentional in direction, unverified in size** — report it alongside the other pacing deltas; do not re-tune here |
| `gen_quest`'s tier bell (`content.gd:676`, `soft_hi`) | the asked-tier bell's centre, which climbs to the band top and saturates | `soft_hi` saturates at L16 regardless of the cadence change (it is a pure level formula) — but L16 now falls inside Snowy Village (L8–19) rather than mid-Desert Oasis, since the scene windows moved | **intentional**; the tier ramp itself is untouched, only which scene the player is in when it finishes ramping |
