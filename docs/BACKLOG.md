...
- ✅ **Buy an item from the board's item info bar — SHIPPED as T55** (2026-06-23, `tasks/ux-feel.md`).
  Tapping a regular board item shows a green **Buy** chip beside Sell; on tap it spends the currency
  and places a live COPY on the board (the bag when the board is full). Dev call "mirror selling +
  markup": new `G.buy_price(code) = ceil(sell_reward × BUY_MARKUP)` (coins sub-top, 💎 top;
  `BUY_MARKUP=3.0` owner dial) — strictly above sell on every tier, so the buy-low/sell-high loop is
  impossible. Honors §4 (you can only buy a copy of an item already on your board). **Parked tail:**
  buying a copy of a *bag* item (the info bar populates from board taps; the bag is a separate overlay).
