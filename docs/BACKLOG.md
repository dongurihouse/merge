...
- ✅ **Reintroduce the burst-upgrade buy UI — SHIPPED as T54** (2026-06-23, `tasks/ux-feel.md`). Dev design call: the buy lives on TWO surfaces — the board **info bar** when the generator is tapped (a green "Boost / 🪙cost" chip in the slot the sell button leaves empty for generators; affordable/dimmed/hidden-at-max) + a coin-priced **water-shop** card ("Boost" under "Bigger bursts"). One shared seam `G.try_upgrade_burst()` (content.gd) drives both; `board.gd._upgrade_gen_burst` delegates to it. Code-built chip (StyleBoxFlat, the redesign's cream/Rest-plane language).
