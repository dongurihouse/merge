...
*Surfaced 2026-06-16 — Shop-screen design review against two reference cozy/merge shops. ✅ **The storefront UX pass SHIPPED as T46** (`tasks/ux-feel.md`): de-grey the cards + green BUY pill (white-on-`Pal.BTN_PRIMARY`), hero icon-on-plate, bolder honey section banners, red claimable dots + a real daily-rotation countdown, IAP cluster-scaling + Popular/Best-value badges + "First buy x2" copy, and a red circular close. Self-contained in `shop.gd` + `tuning.gd` `class Shop`; verified by composite/zoom captures incl. a forced empty-wallet state + the full headless suite. Only the three tails below remain.*

- **Shop item info — the "i" badge DETAIL POPUP (the live sheet; the glyph already ships).** The decorative blue "i" badge now sits on every product card (T46, `shop.gd` `_info_badge`, input-transparent → a tap still buys). What's parked is the **live detail sheet** it should open (item name · what it does · tier preview · where it's used). ⚠️ **Build note:** the whole card presses to **buy** (`_card_button` → `_try_buy`), so the "i" must become a **separate hit target stacked above** the buy press (its own `mouse_filter` + z-order) that opens the sheet **without** triggering a purchase. *(Owner parked the hookup separately — 2026-06-16.)*

- **Shop icons — commission cleaner kit art (art lane · owner).** T46 enlarged the product icons and seated them on a honey disc plate, which lifts the existing `ui/kit/icon_*.png` glyphs a lot, but the underlying art is still thin/engraved (the acorn/coin reads as a grey medallion). Generate **cleaner, chunkier kit icons** (water can · acorn pouch · honey jar · garden tools · mushroom) via the §16 pipeline to match the `PieceView` preview fidelity. *(Surfaced 2026-06-16 — T46 tail.)*

- **Shop badges — consolidate onto the shared "sticker" badge.** T46's red dot / "i" / countdown / "Popular" chips are **self-contained in `shop.gd`** (built before the shared badge component landed). The shared sticker-badge recipe is now in `main` (`skin.gd`, the HUD/chrome *Badge system* item). Fold the shop's local badges onto it for one consistent badge language, and consider the *money-vs-soft-currency* colour split (T46 ships all `$` packs on the same green buy pill — flag if real-money should read differently). *(Surfaced 2026-06-16 — T46 tail.)*
