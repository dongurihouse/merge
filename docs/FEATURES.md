...
| `cascade` | player-tipped adjacent ladders auto-merge into one rewarded cascade chain | `engine/scripts/scenes/board.gd` `_prepare_chain()`, `_run_chain_step()`, `_publish_guide()` | keep — default ON, owner review pending |
