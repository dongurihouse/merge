...
- **What drives the comeback:** energy refills (the soft wall = the return hook) · **an unfinished, evolving world** (the strongest pull — *"my world isn't done and I have an idea for it"*) · **the populated world you've restored** (spirits to bring home from an expedition, and a habitat that produced while you were away, §8) · **events staged in maps you've restored** (§17) · new content always a few steps ahead.
...
> **Two surfaces for the merge verb.** The board's merge is **drag-merge** on the grid (player-driven, above). The **Residents** layer on completed maps (§8) reuses the *same* drag-merge verb on a **second surface — an in-hand tray**: two same-kind+tier spirits drag together to climb a tier *before* they're placed, and placement onto a map is **merge-free** (maps just hold spirits and produce). Same verb, different surface — no second merge *rule*, no on-board second merge. *(An earlier base-game model had residents **auto-merge** silently on the map; the expansion replaces that with player-driven in-hand merge.)*
...
**Corollary — the Residents expansion (§8) reverses the cosmetic-only stance.** The base game treated residents as cosmetic-only (no yield, no power) — the looks side of this line. The Residents expansion gives residents an **economic + progression role** (an idle production faucet, bounded by the shared-bucket capacity — §8): placed spirits now *produce* coins, energy, and premium currency back into the game. That reopens the base economy (the I2 energy invariant and the premium/IAP guard), so it is **re-validated on `grove_sim` plus a Rush sim**, not assumed — the parked §8/§10 economy pass. *(The surprise-capsule rule below is revisited there too.)*

**The bounded surprise-capsule clause (permits a premium random capsule — only under seven guardrails).** A game **may** ship a premium hard-currency **surprise-capsule** (a randomized pull yielding special characters) **only** if it carries all seven cozy guardrails: **(a) cosmetic-only forever** — no yield, no power; **(b) no-loss randomness** — every pull is *wanted*, and **dupes auto-convert** to merge-fuel / soft-currency, never wasted; **(c) no dangled rarity tiers**; **(d) no pity timer**; **(e) evergreen** — no time-limited / FOMO capsules; **(f) soft, transparent pricing with a free/earned path** to the *same* collection; **(g) diegetic framing** — never the word "gacha", and **not bolted onto the no-predatory peddler/merchant** character. Absent any one of these the capsule is **not permitted**. *(This is the engine-level rule a game instantiates; the grove parks its instance post-v1 behind a readiness gate — `grove_spec §1`, `BACKLOG.md`.)* *(Revisited by the Residents expansion (§8), which gives earned spirits an economic role and a later **rarity** axis — so guardrails (a) cosmetic-only and (c) no rarity tiers are scoped to the **premium capsule**, not the skill-earned habitat spirits.)*
...
> `grove_data.gd` (the tables), `grove_sim.gd`. **A–E are now all SHIPPED (2026-06-26).**
>
...
> **retired**, no key is needed — for a **COINS-ONLY** payout **ROLLED** from that tier's
> `CHEST_OPEN_COINS` range, low-biased as `lo + span·randf()^CHEST_ROLL_SKEW`; at skew **3** the
> expectation sits at **1/4 of the span**, so a typical open pays near the floor and the ceiling is the
> jackpot. A chest pays **no acorns and no premium** — its coins are spendable but never clock-advancing.
> Each tier's ceiling and mean more than **double** the tier below, so merging chests up beats
...
### Completed maps → the Residents expansion (Explore · habitat · payback)

A finished map is not an end — completing it **grows the habitat the player populates**, turning the dead "map complete, nothing left to do" state into the game's **primary long-tail and daily-return loop**. Restoration spots are **unlock-once** (a cumulative exp threshold §3, claimed free, ruined→**restored** — no spend, no renovation ladder); once **all** of a map's spots are claimed the map **completes**, and completion **grants cells to the one global habitat bucket** (the shared roster surface — there are no per-map rosters). The loop bolts onto the merge core without changing how the core plays; it is the engine's **open-ended soft-currency sink** *and* its **anti-abandonment "living world"** layer — it replaces the older home-hub coin-upgrade→passive-yield loop (deleted with its keystone coin *sink*, its passive coin *faucet*, and the "building grows richer" beat — an accepted loss). Four moves form a self-renewing flywheel — **Explore → Place → Expand → Reward**:

- **Explore — acquire (a second, net-new board engine).** A global **destination** the player ventures out to (not a per-map shop). Three beats — **Load out → Rush → Trade.** *Load out:* spend **coins** on stackable **boost items**, consumed per run (the open-ended **coin sink** — coins' first unbounded use). *Rush:* a short timed (~45–90 s) **merge-for-score** game on a **dedicated, temporary board** (discarded after — never the home board): traces **rain in automatically** (no generator, no energy) and accelerate; **tap-to-merge** a neighbour and the result **rerolls to a random line** (you merge for *tier*, not a tunneled line); **fling** a no-match tile to a safe column; a telegraphed **hazard** periodically destroys a column (emptying it first = a multiplier bonus); **score = non-linear-by-tier × a live combo multiplier**; two clocks (countdown + board-full) end the run. *Trade:* the score buys **mystery boxes** of escalating cost/odds; each box reveals **spirits** that come home. Boxes are **skill-earned, not sold** — every tier is reachable by playing well, and any premium path buys **better odds, never an exclusive character** (the §4 "buys speed/odds, never possibility" law). The Rush is its **own board engine** (auto-drop · random-line tap-merge · fling · hazard · scoring) — the single largest build item, not a reskin.
- **Place — the shared bucket (in-hand merge, then place).** The habitat is **one global bucket of cells** (starts at 0; each completed map grants cells — a map's completion reward *is* its capacity grant; **capacity is never sold for currency**). Box-spirits land **in-hand** (an unbounded holding area); the player **merges two-of-a-kind in hand** (same **kind + tier** — the same drag-merge verb, on a second surface; §2) up a **full 12-tier housed ladder**, then **places** them into a cell. **One cell per spirit, any tier, duplicates of a kind welcome**, freed only by **selling or moving** (never by merging — placement is merge-free; a higher-tier spirit packs more production into one cell). **Capacity is the brake** — the reward **lines** (below) compete for the same few cells, so the reward mix is a trade-off the player chooses; **selling is the always-available pressure valve** (even an all-singletons bucket can free a cell).
- **Expand — the pull outward.** Cells come **only from completing maps**, so running out of room becomes a concrete reason to push the base-game progression (the expansion rides the existing completion chain — no new unlock mechanic); merging is the other axis — a merged spirit carries the same production weight in fewer cells, so **tiers buy cell efficiency**, the scarce thing.
- **Reward — the payback (per-line dials, bounded by choice).** Every spirit kind belongs to a **line**, and each line pays one resource (soft currency · energy · a generator-boost charge · premium currency — the line table is a game instance, `grove_spec §3`); a line's output is driven by its **Σtier** (the sum of placed tiers of that line). Each line gets its **own dial** for what tier does: free-scaling on the sink-positive soft-currency line, small-banked top-up on energy, and on the premium line tier shortens the maturation timer + deepens the bank while the **per-day yield stays a designed ceiling**. **Merge always pays (hard invariant):** every merge perceptibly improves at least one of **cadence** or **bank cap** on its line — no merge is a dead upgrade, even on bounded lines. Acquisition boxes roll a **line by weight** (premium-line drops rare — climbing that 2^(n−1) ladder is gated by rarity, not a felt cap). Production is **idle and banked**; the player **collects** from the habitat surface. The same spirits also fill a **Collection** — a global almanac of discovered spirits (parked; sets + a completion reward are later layers). *(A spirits-produce-spirits line is **disallowed** — it self-compounds and competes with the paid Explore, the coin sink.)*

*(Rarity is a clean **later extension**, parked: v1 spirits are all one rarity and production is **tier-only**; the rarity axis — white · blue · orange · red, box-weighted, yield-scaling — and the collection almanac return together. Quest-giving by residents was considered and parked: residents are the *supply* side, the quest fence is *demand*.)*

**This reverses three base-game invariants — re-proven, not inherited.** The base economy treated residents as a cosmetic-only, **uncapped** coin sink with no passive faucet. The expansion makes the sink **finite (capacity)**, adds an **idle production faucet**, and lets lines mint **energy and premium currency** — so `sink > faucet`, the energy invariant (I2), and the premium/IAP guard must be **re-validated on `grove_sim` plus a new Rush sim** (fair tide · score→box pacing · no dominant strategy · no forced loss), not carried over. That re-author is the **parked economy pass** (§10). **Containment is by construction and by choice:** all lines share the one small bucket (maxing one means zeroing the others — the total Σtier ceiling is cells × max-tier, not per-map multiples of it); the energy line is a small-banked top-up (I2-safe); the premium line's per-day yield is a designed ceiling behind rare drops; and there is **no free-spirit line**, so paid Explore stays the only spirit source.

- **The roster is the source of truth — not the display.** Membership is the **persisted global bucket** (capacity-bounded) plus the hand, *not* the stateless on-screen crowd: who lives in the world survives a session. Maps keep an **ambient flavour crowd** (a decorative layer, §12), decoupled from production; a parked polish item renders the **placed roster wandering whichever map the player is viewing**, so the one roster looks at work everywhere. The visible populace stays sparse under §16's **"never a dense single render"** and the §12 **Calm-Mode** particle budget even as the lifetime roster grows. *(Migration is non-destructive — spirits from a retired per-map roster return to the hand; completed maps grant their cells immediately.)*

**The home hub is now a narrative + functional anchor, not a unique mechanic.** Every restored map grows the shared habitat, so the Residents loop is *general*, not the hub's. One map is still the **home hub** (the game designates it — typically the **first**; the grove's homestead): it restores and completes like any map, but stays special through **narrative/functional anchors** — the **HUD "home" shortcut** that jumps back from anywhere, **deeper authoring** (a richer scene than a finish-once map), and any **story spine** sited there — **not** a mechanic other maps lack. *(Ongoing per-map **yield** and resource **feed-forward** — old maps continuously paying into the new — were considered and **parked** as a 10-map chore; the Residents loop is the chosen "old maps stay alive" answer instead. The one-time **generator hand-in** at a boundary (§6/§7) is *not* that loop: it's a single, forward-flowing step of progression, so it stays. BACKLOG.)*
...
- **Restored maps stay inhabited — and grow more so.** A finished map keeps its **ambient life (§12)**, is always revisitable from the select, and **grows the habitat bucket** (above) — the player keeps bringing spirits home, merging them in hand, and placing them for the life of the save, and each finished map keeps its ambient crowd (plus, parked, the wandering placed roster) — a **living world that keeps growing**, not a chain of spent levels.
...
> **Reward model B (partially live).** The active-loop-buff family — upgrades that improve *merging*, not just the world — has its **first piece live: generator burst-upgrades** (§6 — pay coins/premium to pop more per tap), a **board-level coin sink that scales as maps add lines**, independent of the Residents loop. The rest of *this* family (a leveled generator pops better tiers / cheaper energy) stays **parked, not v1**. *(Distinct from the **Residents** loop above — that is the keystone endgame loop (Explore → place → produce on completed maps); "reward model B" here is only the **merge-affecting** buff family.)*
...
THE SPINE — the loop the economy spins on
  ...
  ✦ EXP (cumulative, never spent) ─┬─► ▲ LEVEL   (each level grants ⚡; 💎 only at milestones)
                                   └─► crosses spot THRESHOLDS → claim spots FREE = BUILD the maps → a COMPLETED map GROWS the shared HABITAT bucket (Residents loop)
                                                                    │
  🪙 COINS ─┬─ spent → EXPLORE load-out + map CAPACITY upgrades (open-ended sink, §8) · burst-boosts · cosmetics
            └─ from → merge drops · selling (every tier) · quest coins · PLACED-resident production (§8)

  💎 DIAMONDS (= acorns) — premium, EARNED-ONLY at milestones + live IAP from launch · buys speed + looks, never possibility
       └─ earned → map-complete · level milestones · capped map-4 trickle (§8)   sink → energy refill · bag slots · premium EXPLORE odds · (post-v1) the surprise-capsule
```
...
| **Coins 🪙** (soft) | merge drops (~10%) · selling **every tier** · quest coins · shop pack · **placed-resident production (§8 — e.g. a map's coin reward)** | **Explore load-out boosts + map-capacity upgrades (§8 — the primary, open-ended coin sink)** · **generator burst-boosts (§6)** · customization · basket buy-back | The **soft-economy** currency — it funds Explore + capacity + burst-boosts, so it has real power (not dead cosmetics). **Sink topology is OPEN-ENDED** — the load-out is consumed every run, so coins always have a next run to fund (capacity is a finite second sink). Coins are now also a **faucet** (resident payback), so sink-positivity holds against the **total**. |
| **Diamonds 💎** (premium, = **acorns**) | **earned-only**: level-up **milestones** · map completion · a **hard-day-capped placed-resident trickle** (§8 — one map's reward, the IAP-guarded faucet) · **cash packs (live IAP from launch)** · rewarded ads — *never* from quests or selling | energy refill · bag slot · cosmetics · **premium Explore — better box odds / premium load-out (odds + speed, never an exclusive spirit, §8)** · (post-v1) **the surprise-capsule (§4 guardrails)** · starter / first-buy packs | Premium-**shaped** and **precious** (peg: 1 acorn = `COINS_PER_ACORN` coins); **earned only at milestones + live IAP from launch** (geo-flagged), the §8 map-trickle hard-capped. **Buys speed + looks, never possibility.** |

### The soft-currency loop (coins)

Soft currency **flows in** from merge drops + selling + quest coins — and, with the Residents expansion (§8), from **placed-resident production** (one map pays coins back). It **flows out** into the **Explore load-out** (boost items consumed every run — the open-ended sink, coins' first unbounded use), **map-capacity upgrades** (a finite second sink), **generator burst-upgrades** (§6), and customization. The governing law still holds — **sinks exceed the faucet** — but two things shifted from the old hub model: the open-endedness now comes from the **per-run consumable load-out** (there is always a next run to fund), *not* from an unbounded resident roster (residents are now capacity-capped, §8); and coins are now **both sink and faucet**, so sink-positivity must hold against the **total** coin faucet (drops + selling + quest coins + resident payback), re-proven on `grove_sim`, not in isolation. The sink is **functional, not cosmetic** — and **diegetic**: venturing out and welcoming spirits home, never a bare store (§13). *(The grove's instances — load-out items + prices, capacity-upgrade costs, the per-map production rates: see `grove_spec`.)*

> **Resolved (was a standing tension):** earlier, coin sinks were cosmetic-only, so the *motivation* to spend coins was thin ("coins have no power"). The fix is the **Residents expansion** (§8) + **generator burst-upgrades** (§6): the **Explore load-out** is coins' first **open-ended** use (boosts consumed every run), **map-capacity upgrades** are a second sink, and placed spirits **pay coins back** — so coins fund a real endgame loop with pull, off the "premium buys speed" line. The sink is **open-ended** (a next run always awaits) rather than the earlier finite hub ladder; and because residents now also *produce*, `sink > faucet` is re-proven against the **total** faucet on `grove_sim` (§8). *(The earlier home-hub upgrade→passive-yield loop was the previous answer — now **removed**; per-map yield before it was parked as a collect-from-every-map chore. `BACKLOG.md`.)*
...
- **2× expedition** — a doubler on one Explore run's score, or a discounted load-out (§8) per cycle (the Residents loop's optional faucet-side nudge).
...
- **Other types, same framework (tuned knobs):** **bonus weekends** (×2 coin drops / cheaper energy), **limited cosmetics** (event-only skins in the Shop, §10), **catch-up bundles** (a discounted spot pack for returning players), **event visitors** (ephemeral resident visitors **sited per restored map** — a time-boxed guest that wanders a completed map's habitat §8 during the event window, then leaves; a fresh face that refreshes old maps without permanently growing the roster).
...
| **New spirits / visitors** | new spirits are home from an expedition, or an event visitor wanders a restored map's habitat (§8) — come place / see them |
| **Habitat ready to collect** | a completed map's placed spirits have banked their production (§8) — a soft "your world worked while you were away" beat |
