...
- **What drives the comeback:** energy refills (the soft wall = the return hook) · **an unfinished, evolving world** (the strongest pull — *"my world isn't done and I have an idea for it"*) · **the populated maps you've restored** (spirits to bring home from an expedition, and habitats that produced while you were away, §8) · **events staged in maps you've restored** (§17) · new content always a few steps ahead.
...
- **Place — capacity (in-hand merge, then assign).** Each completed map is a **habitat with a slot capacity** (engine default ~8, upgradable). Box-spirits land **in-hand** (an unbounded holding area); the player **merges two-of-a-kind in hand** (same **kind + tier** — the same drag-merge verb, on a second surface; §2) up a **full 12-tier housed ladder**, then **assigns** them onto a map. **One slot per spirit, any tier**, freed only by **selling or moving** (never by merging — placement is merge-free; a higher-tier spirit packs more production into one slot). Re-assignment is free, and a map's output sums all its assigned spirits. **Capacity is the brake** — only your ~8 best fit on a favourite reward, so a growing roster *must* spill onto other maps (a natural reward mix); **selling is the always-available pressure valve** (even an all-singletons habitat can free a slot).
- **Expand — the pull outward.** Capacity is **per map**, so the player **upgrades** a map's capacity or **unlocks the next map** — each new map brings its own starting slots *and* a **new reward stream**. Running out of room becomes a concrete reason to push the base-game progression; the expansion rides the existing completion chain (no new unlock mechanic).
- **Reward — the payback (idle, capped).** Placed spirits **produce for the core game** — a **distinct, non-staling reward per map** (coins · energy · a generator-boost charge · premium currency · more spirits — the per-map table is a game instance, `grove_spec`). The production model is decoupled to stay invariant-safe: **each map matures a FIXED reward unit; a placed spirit's TIER speeds the cadence and COUNT raises the accrual cap — the per-unit amount never scales** (stacking high tiers collects *faster*, not *more*, so the energy/premium streams stay bounded). Production is **idle and capped** (a daily-return ceiling, not infinite idle); the player **collects** from the habitat surface. The same spirits also fill a **Collection** — a global almanac of discovered spirits (parked; per-map sets + a completion reward are later layers).
...
**This reverses three base-game invariants — re-proven, not inherited.** The base economy treated residents as a cosmetic-only, **uncapped** coin sink with no passive faucet. The expansion makes the sink **finite (capacity)**, adds an **idle production faucet**, and lets a map mint **energy and premium currency** — so `sink > faucet`, the energy invariant (I2), and the premium/IAP guard must be **re-validated on `grove_sim` plus a new Rush sim** (fair tide · score→box pacing · no dominant strategy · no forced loss), not carried over. That re-author is the **parked economy pass** (§10). **Containment is by construction:** the energy stream is a fixed top-up that scales only in cadence (I2-safe); the premium faucet is hard-day-capped against the IAP ladder; the free-spirit faucet banks slowly so paid Explore stays primary.

- **The roster is the source of truth — not the display.** Membership is a **persisted per-map roster** (capacity-bounded), *not* the stateless on-screen crowd: who lives here survives a session, and the wandering sprites are a *render* of the roster. The visible populace stays sparse under §16's **"never a dense single render"** and the §12 **Calm-Mode** particle budget even as the lifetime roster grows. *(Migration is non-destructive — an old cosmetic-only per-map roster reads as housed tier-1 spirits at default capacity.)*

**The home hub is now a narrative + functional anchor, not a unique mechanic.** Every restored map is a habitat, so the Residents loop is *general*, not the hub's. One map is still the **home hub** (the game designates it — typically the **first**; the grove's homestead): it restores and completes like any map, but stays special through **narrative/functional anchors** — the **HUD "home" shortcut** that jumps back from anywhere, **deeper authoring** (a richer scene than a finish-once map), and any **story spine** sited there — **not** a mechanic other maps lack. *(Ongoing per-map **yield** and resource **feed-forward** — old maps continuously paying into the new — were considered and **parked** as a 10-map chore; the Residents loop is the chosen "old maps stay alive" answer instead. The one-time **generator hand-in** at a boundary (§6/§7) is *not* that loop: it's a single, forward-flowing step of progression, so it stays. BACKLOG.)*
...
- **Restored maps stay inhabited — and grow more so.** A finished map keeps its **ambient life (§12)**, is always revisitable from the select, and **opens its habitat** (above) — the player keeps bringing spirits home, merging them in hand, and placing them there for the life of the save, so a completed map is a **living place that keeps growing**, not a spent level.
...
THE SPINE — the loop the economy spins on
  ...
  ✦ EXP (cumulative, never spent) ─┬─► ▲ LEVEL   (each level grants ⚡; 💎 only at milestones)
                                   └─► crosses spot THRESHOLDS → claim spots FREE = BUILD the maps → a COMPLETED map opens its HABITAT (Residents loop)
                                                                    │
