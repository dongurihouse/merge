...
Before generating or accepting a new line sheet, start from the reusable prompt in
`docs/design/grove_item_line_prompt.md` and apply the cutter-first art requirements in
...
shadows, no floor/grounding, no sparkles/glow/detached FX, mostly single-object tiers, compact
everyday objects, distinct silhouette/material families from neighboring lines, and high-tier acorn
signature motifs placed in natural emblem slots when the object family supports them.
...
first/last rows and columns byte-identical. The committed, export-excluded
`games/grove/assets/_review/ui/meadow_v2/*_contact.png` files prove row-major identity; every
`*_3x3_offset.png` exposes both texture boundaries through the middle of the review image and must be
fully opaque, gap-free, and free of magenta. Review montages never live below production `assets/ui`
and `export_presets.cfg` excludes the entire `_review` tree from builds.
