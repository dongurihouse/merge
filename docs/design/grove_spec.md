...
*Instantiates Core §8's Residents expansion + Core §10's soft-currency loop.* **This replaces the cut home-hub Coins-upgrade→passive-yield loop** (§1 Scope) — there is **no yield faucet** and **no L1→L5 coin-upgrade axis**; spots simply restore ruined→**restored** once (exp threshold, §4), and a **completed** map becomes a **habitat** the player populates. It works on **every completed map**, not just the hub, turning "map finished, nothing to do" into the game's primary long-tail — the grove's build of Core §8's **Explore → Place → Expand → Reward** flywheel. **Status (working title "The Homecoming"):** the Explore rush + mystery boxes, the in-hand habitat payback, and **all five map reward streams** are BUILT with provisional, hard-capped numbers (`engine/scripts/core/habitat.gd`); **rarity, the collection almanac, and the economy / Rush-sim re-author are parked** (numbers sim-tuned last).
...
- **Place — the habitat (capacity).** Each completed map is a **habitat with a slot capacity** (~8, upgradable) — and **the map screen itself is the management surface**: a **spirits dock** above the bottom nav holds in-hand spirits and the verbs **merge · assign · collect · free**. Box-spirits land **in-hand** (unbounded); the player **merges two-of-a-kind in hand** (same **kind + tier**, a drag — climbing a **full 12-step housed ladder**, `RESIDENT_MAX_TIER`), then **assigns** them onto the open map (placement is merge-free). **One slot per spirit**, freed only by **sell or move**; re-assignment is free; a map's output sums all its placed spirits. **Capacity is the brake** — your ~8 best fit one favourite reward, so a growing roster spills across maps (a natural reward mix); **selling is the always-available valve.** *(This supersedes the per-map welcome panel; the legacy welcome-shop + its `resident_counts` roster are kept dormant until the economy pass.)*
- **Reward — the payback (idle, capped; all five wired).** Placed spirits **produce for the core game** — each completed map a **distinct, non-staling** reward. The model: **each map matures a FIXED reward unit; a placed spirit's TIER speeds the cadence and COUNT raises the accrual cap — the per-unit amount never scales** (so the invariant-sensitive streams stay bounded). Production is **idle + capped**; you **collect** from the map-select card + the residents dialog. The five home-grove streams (all built, provisional hard caps):

  | Map | Produces | Note |
  |-----|----------|------|
  | 1 Farmhouse | **Coins** | coins' payback (and self-funds the Expedition load-out) |
  | 2 Orchard | **Water** | a fixed top-up clamped to `WATER_CAP` — I2-safe (only cadence scales) |
  | 3 Old Mill | a **generator-boost charge** | stockpiled, armed free (reuses the §2 temporary boost) |
  | 4 Wildflower Meadow | **Diamonds** | strict per-day hard cap — the IAP guard |
  | 5 Vale Gate | a **resident chest** | collecting drops 1 / 4 / 8 spirits to the hand (size by placed count) |

  The map-5 chest reuses the **same 1/4/8 box tiers as the rush Trade** (one `Habitat.grant_chest` path). Exact rates/caps are the parked economy detail.
- **Expand — the pull outward.** Capacity is **per map**, so the player **upgrades** a map's capacity or **unlocks the next map** (each new map = its own ~8 slots *and* a new reward stream). Running out of room drives base-game progression. *(Runway: only the 5 home-grove maps are wired today against the designed 20-place journey, below — the Expand arm stays thin until post-launch maps ship.)*
- **Collection (parked).** A **global almanac** of discovered spirit-folk (the later rarity axis × kind) on its own shared-almanac page — parked to `BACKLOG.md`; per-map sets + a completion reward are later layers.

**v1 simplification — rarity is parked.** v1 spirits are all **common** and production is **tier-only**. The **rarity axis** (white · blue · orange · red — box-weighted, yield-scaling), the merge-across-rarity rule, and the collection almanac are a clean later extension that re-keys the roster to **(kind, rarity)**; wherever the spirit art (§9) or boxes mention rarity, read v1 as the all-common version.
...
**Invariant reversals (re-proven on `grove_sim`, §5).** The expansion reverses three base assumptions: **capacity** makes the resident sink finite (the old `sink > faucet` proof assumed it was uncapped), **idle production** reopens a passive faucet, and maps 2/4 mint **Water and Diamonds** — so I2 and the IAP guard must be re-validated, not inherited. Containment is by construction (Water a fixed cadence-only top-up; Diamonds hard-day-capped; the map-5 free-spirit faucet slow so paid Expedition stays primary). The re-author — `grove_sim` around the load-out + capacity + production faucets, plus a new **Rush sim** — is the parked §5 economy pass.
...
- **Restored maps stay inhabited — the Residents loop *is* this.** A completed map opens its **habitat** (above): the player keeps **bringing home, merging in hand, and placing spirit-folk** there for the life of the save, so an old map is a **living place that keeps growing**, not a spent level — its **persisted roster** (§7) is who lives there. This is the Keepers fiction (§1) made playable: tend a place → its folk come home.
...
| **Coins 🪙** | merge drops (~10%) · selling **every tier** (§9) · **quest coins** (§4) · **placed-resident production** (§3 — map-1 reward) · Shop **5💎→150🪙** | **Expedition load-out boosts + map-capacity upgrades** (§3 — the primary, open-ended coin sink) · **generator burst-boost** (§2) · waysides · spirit treats · basket buy-back |
| **Diamonds 💎 (acorns)** | **earned-only & precious** (1 acorn = `COINS_PER_ACORN`=1024🪙): level **milestones** (+3 every 10) · map complete (+5) · a **hard-day-capped map-4 resident trickle** (§3, the IAP guard) · **piggy crack** (cash) · **cash packs** (live IAP) — *never* quests, *never* selling | Water refill (25💎) · Bag slots 7–18 (premium each, Core §5) · gem variants (2–4💎) · **premium Expedition odds** (§3 — better box odds / premium load-out, never an exclusive spirit) · (post-v1) **the surprise-capsule** (§1, Core §4) · **Shop item-shortcuts / exclusive cosmetics** (below) |
...
| **Map-capacity upgrades** | escalating coins per +slot (sim-tuned) | per map (finite) | — (core loop) | **functional** — more habitat room for placed spirits (§3) |
...
A plot is dormant (greyed) until its map is fully restored, then shows a coin-cost pin; buying is **coin-only, never level-gated, in no unlock chain**. **Faucet vs sink:** the lifetime coin faucet is merge drops + selling + **quest coins** (Core §7) + **placed-resident production** (§3, map-1's reward) — **no hub yield any more** (the old passive faucet is cut, §3); the sinks — the **open-ended Expedition load-out** + **capacity upgrades** (§3) + the generator-burst ladder (§2) on top of the cosmetic waysides (~1.9 k) — **exceed it** (sink > faucet, *with teeth*), and because the load-out is consumed **per run** there is no finite ceiling for the faucet to catch. Because coins are now **both sink and faucet**, sink-positivity is re-proven against the **total** in the §5 economy re-author. *(Premium Expedition odds are a separate **diamond** sink, §3 — not coins.)*

> **The "coins have no power" tension — resolved by the v1 Residents expansion (§3, Core §8/§10).** The *legacy* grove sinks (waysides, treats) were all cosmetic / net-zero, so motivation to spend was thin — a decorator pull only. The fix is now **v1**: the **Expedition load-out** (coins kit out the merge-rush — the open-ended sink) + **map-capacity upgrades** plus **generator burst-upgrades** (§2) give coins real power, off the "premium buys speed" line; the cosmetic sinks stay as the **décor layer** on top. And unlike the old model, placed residents now **pay coins back** (map-1 production, §3), so coins are both sink and faucet — sink-positivity is re-proven against the total. *(Load-out / capacity / production rates are sim-validated in the §5 re-author. The earlier home-hub upgrade→passive-yield loop is **removed** — §1 Scope. The legacy coin-gated bedroom-decor sink — 663🪙 in `districts.gd`/`room.gd` — was retired with the open-space-map model.)*

> **The economy is REOPENED (a fresh balance pass, not a tweak).** Cutting the hub loop deletes both a keystone coin sink (~8,600🪙 ladder) **and** the passive coin faucet (96🪙/day), and the Residents expansion (§3) bolts on new flows — so the §5 numbers need re-authoring, not adjusting. The open work: **(1)** re-author `grove_sim` around the **Expedition load-out + map-capacity** coin sinks and the **per-map production faucets** (incl. coins from map 1), plus a new **Rush sim** (fair tide · score→box pacing · no dominant strategy · no forced loss); **(2)** model the **diamond economy** — premium Expedition odds + the **hard-capped map-4 diamond faucet** (the most-scrutinized number, against the IAP ladder) alongside bag slots + gem variants, sized to the gentle cozy-ARPU posture (§1); **(3)** cover the **early-game window** — the expansion only opens once the **first map completes**, so pre-first-completion coins have only burst-upgrades + cosmetics to spend on; verify coins aren't dead there; **(4)** re-derive the **★ scaffold** (the retired 31★/176★ figures, §3) on the unlock-once spots; **(5)** re-prove the reversed invariants (capacity makes the sink finite; idle production + Water/Diamonds reopen I2 + the IAP guard, §3). All hard invariants stay (no-strand, no-jam, sink > faucet, selling-not-income, I2) — re-validate every dial on `grove_sim` + the Rush sim.
...
- **Spirit-folk** are the grove's ambient figures — and on a **completed** map they *are* its **placed residents** (§3 Residents expansion, Core §8). Membership is the **persisted, capacity-bounded per-map roster** (in the save) — the spirits the player merged in hand and **assigned** here — **not** a fixed `1 + restored maps` count; the wandering sprites are a *render* of that roster. Tap → `hop`. The **"crowd as it heals"** payoff (§1) now comes from the **Expedition-and-place loop** plus **per-place variety** — the **map-specific rosters** (§3: a shared core of ~3 recolorable wanderers + ~2 map-signature specials each), echoing §1's per-place leads + rotating menagerie. On an un-completed map a simpler ambient count applies (pre-completion the habitat isn't open yet). They're the **"restored maps stay inhabited"** life (§3 anti-abandonment) and the build-reveal **crowd** (a §6 incoming flag). *(`ambient_characters` flag §6 — its `1 + restored maps (cap 5)` rule is **superseded** by the habitat roster model for completed maps.)*
