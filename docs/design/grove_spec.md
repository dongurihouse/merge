...
**Burst + the temporary boost (the second functional coin sink).** A generator tap pops a **burst** whose odds scale with the generator's **tier** (Core §6 `GEN_TIER_BURST_ODDS`). The coin sink is the **temporary per-generator boost**: a flat **120🪙** (`BOOST_COST`) arms **10 taps** (`BOOST_TAPS`) of strictly-better boosted odds (`GEN_TIER_BURST_ODDS_BOOST` — the top tier gains a **4th burst slot**) — per-generator, stackable across generators, **repeatable forever**: the grove's **second functional coin sink** alongside the Residents loop (§3). It cuts **taps, not the per-item energy cost**, so the energy monetization socket (Core §4) and the pacing curve (Core §3) stay **untouched**.
