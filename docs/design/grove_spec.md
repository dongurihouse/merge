...
- **The Residents expansion *is* the Keepers fiction made playable (§3/§5).** Restoring a place doesn't just refurnish it — **its spirit-folk return to live there.** A *completed* map **grants cells to the shared habitat** (§3 — one global bucket, not a roster per map); the player **ventures out on an Expedition** to find spirit-folk, **brings them home**, merges them in hand, and **places** them to live and produce. Acquisition is diegetically **welcoming a spirit home** (Core §13, the commerce-dressed law) — care made visible, the same fiction as restoration itself. A world the Keepers tend is a world **filling back up with the folk who belong there.**
...
**Burst + the temporary boost (the second functional coin sink).** A generator tap pops a **flat 1–3 item burst** (`BURST_ODDS`). The coin sink is the **temporary per-generator boost**: a flat **120🪙** (`BOOST_COST`) arms **10 taps** (`BOOST_TAPS`) of strictly-better flat boosted odds (`BURST_ODDS_BOOST`) — per-generator, stackable across generators, **repeatable forever**: the grove's **second functional coin sink** alongside the Residents loop (§3). It cuts **taps, not the per-item energy cost**, so the energy monetization socket (Core §4) and the pacing curve (Core §3) stay **untouched**. Generator rank/mastery affects the item-tier pop window, not a separate generator merge tier.
...
> **Build vs the design target.** The *current* grove implements the **build** half (unlock fixed spots at **exp thresholds**) plus the **Residents expansion** (Explore rush + boxes + the habitat payback — `habitat.gd`), now **v1 — the keystone**, specified below (*The Residents expansion*): restoration spots are **unlock-once** (claimed **free** the moment exp crosses the spot's threshold — no spend; ruined→restored, no coin-upgrade axis), and each **completed** map grants cells to the **shared habitat bucket** (§3) the player populates by venturing out on an **Expedition** (merge-rush → mystery boxes → bring spirits home → merge in hand → place to produce). This is what turns a finished homestead from a static decorate-surface into the **retention engine**. *(This **replaces** the previously-planned home-hub Coins-upgrade→passive-yield loop, now cut — §1 Scope.)*
...
### The Residents expansion — venture out, bring spirits home, build a habitat (the v1 keystone endgame)

*Instantiates Core §8's Residents expansion + Core §10's soft-currency loop.* **This replaces the cut home-hub Coins-upgrade→passive-yield loop** (§1 Scope) — there is **no L1→L5 coin-upgrade axis**; spots simply restore ruined→**restored** once (exp threshold, §4), and each **completed** map grants cells to the **shared habitat bucket** (below). Every map completion feeds the one long-tail loop, turning "map finished, nothing to do" into the game's primary retention engine — the grove's build of Core §8's **Explore → Place → Expand → Reward** flywheel. **Status (working title "The Homecoming"):** the Explore rush + mystery boxes and the in-hand payback are BUILT; the built **per-map** habitat model (five per-map reward streams, `engine/scripts/core/habitat.gd`) is **superseded by the global-bucket redesign** (`docs/superpowers/specs/2026-07-16-global-resident-bucket-design.md`, specified below) — the per-map model let all five streams run concurrently at 8 slots × 5 maps and made the ceiling an unbounded faucet. **Rarity-axis, the collection almanac, and the economy / Rush-sim re-author stay parked** (numbers sim-tuned last).

- **Explore — the Expedition (acquire).** The player **ventures out from home** (the bottom-nav **Expedition** button → a Load-out dialog), not a per-map shop. Three beats — **Load out → Rush → Trade.** *Load out:* spend **coins** on stackable **boost items** (extend time · faster drops · calmer woods · tier-2 drops · fewer lines…), consumed per run — the open-ended **coin sink** (§5). *Rush:* the **Expedition Rush** — a short timed merge-for-score game on a temporary board (v1 reuses the home 7×9, discarded after): **traces rain in** automatically (no satchel, no Water) and accelerate; **tap-to-merge** a neighbour and the result **rerolls to a random line**; **fling** a no-match tile to a safe column; a telegraphed **treefall** periodically destroys a column (empty it first = a clean-dodge multiplier bonus); **score = non-linear-by-tier × a live combo multiplier**; the countdown *and* board-full end the run. (Screen layout: *The Rush screen*, below.) *Trade:* the score buys **mystery boxes** of escalating cost/odds that reveal **spirit-folk** to bring home — boxes are **skill-earned** (score, not money), every tier reachable by playing well, and any premium path buys **better odds, never an exclusive spirit**.
- **Place — the shared bucket (capacity).** The habitat is **one global bucket of cells**, detached from any single map: it opens at **0 cells** and each **fully restored map grants cells** (grant schedule a tuning call; ~8 total across the five home-grove maps — a map's completion reward *is* its capacity grant). The **spirits dock** above the bottom nav is the one management surface: cells + the unbounded hand + the verbs **merge · place · collect · sell**. Box-spirits land **in-hand** (unbounded); the player **merges two-of-a-kind in hand** (same **kind + tier**, a drag — climbing a **full 12-step housed ladder**, `RESIDENT_MAX_TIER`), then **places** them into a cell. **One cell per spirit**, duplicates of a line welcome in any number of cells; a cell is freed only by **sell or move**; **selling is the always-available valve.** **Capacity is the brake** — and it comes **only from map completion**: the previously planned coin-purchasable capacity upgrades are **cut** (coins must not buy past the opportunity-cost limiter). *(This supersedes the per-map rosters AND the per-map welcome panel; the legacy welcome-shop + its `resident_counts` roster are kept dormant until the economy pass.)*
- **Reward — four lines, per-line dials (idle, bounded by choice).** Every resident kind belongs to a **line**, and each line pays one resource; a line's output is driven by its **Σtier** (the sum of tiers of all placed residents of that line, across all cells). The four lines — the streams now **compete for the same cells**, so the bound is the player's own trade-off, not a wall:

  | Line | Pays | Tier does | Bound |
  |------|------|-----------|-------|
  | **Coin-kin** | Coins | scales rate + bank cap | none needed — sink-positive vs. the Expedition load-out, re-proven in sim |
  | **Water-kin** | Water | scales rate + bank cap | banks to a small cap — a top-up, never an energy replacement (I2-safe) |
  | **Boost-kin** | generator-boost charges | scales rate + bank cap | small bank cap (reuses the §2 temporary boost) |
  | **Diamond-kin** | Diamonds | shortens the maturation timer **and** raises the bank cap | **per-day yield ceiling is a designed number** (~1–2/day at full dedication, sim-tuned) + rarity-gated drops — the IAP guard |

  **Merge always pays (hard invariant):** every merge must perceptibly improve at least one of **cadence** (time to produce) or **bank cap** (max accrued before a collect) on its line — even on the day-bounded diamond line, no merge is ever a dead upgrade (sim-asserted per tier step). Production is **idle + banked**; you **collect** on the dock (per-line pending badges) — the per-map collect cards **retire**. The old fifth stream (a resident chest — spirits producing spirits) is **cut**: it self-compounds, and the Expedition must stay the only spirit source (it is the coin sink). Boxes roll a **line** by weight (coin-kin common → **diamond-kin rare**: climbing the diamond ladder needs 2^(n−1) copies of a rare drop — rarity, not a felt cap, is the second brake) and a **tier** off the t1-heavy curve. Exact rates/weights are the parked economy detail.
- **Expand — the pull outward.** Cells come **only** from completing maps, so running out of room drives base-game progression: **finish the next map** to grow the bucket. Merging is the other expansion axis — a merged spirit carries the same Σtier in fewer cells, so **tiers buy cell efficiency**, the scarce thing. *(Runway: only the 5 home-grove maps are wired today against the designed 20-place journey, below — the Expand arm stays thin until post-launch maps ship.)*
- **Collection (parked).** A **global almanac** of discovered spirit-folk on its own shared-almanac page — now legible by construction (**4 lines × 12 tiers** of per-tier art) — parked to `BACKLOG.md`; sets + a completion reward are later layers.

**v1 simplification — the rarity AXIS is parked.** v1 spirits are all **common**; the per-line **box drop-weights** above are v1 (they are acquisition odds, not a rarity axis). The **rarity axis** (white · blue · orange · red — box-weighted, yield-scaling), the merge-across-rarity rule, and the collection almanac are a clean later extension that re-keys the roster to **(kind, rarity)**; wherever the spirit art (§9) or boxes mention rarity, read v1 as the all-common version.

**Feel-prototypes (decisions locked).** Two HTML feel-prototypes validated the model: `docs/design/prototypes/expedition_rush.html` (Loadout → Rush → Trade → Manage — locked: tap-to-merge no-drag · fling rescue · auto-rerolled line · score-not-capture · mystery boxes · loadout as the coin sink) and `docs/design/prototypes/habitat_production.html` (the payback half — locked: in-hand drag-merge · drag-to-place · idle fill→collect · capacity freed by sell/move). Open tuning (deferred to the Rush sim + playtest): the score curve + box costs/odds, treefall destroy-vs-knockdown, collect cadence, the accrual-cap curve.

#### The Rush screen — four-band layout (`scenes/explore_rush.gd`)

The Expedition Rush screen reads top-to-bottom as **four bands**, and the whole screen **reflows on a live viewport resize** (mirroring the `map.gd` / board-action-bar pattern — a coalesced `size_changed` → one relayout per frame; stateless chrome rebuilt under a `_chrome` child behind the tiles, the stateful run-grid tiles repositioned, never destroyed):

1. **Top bar** (`Kit.rush_bar`) — Time · Score · Mult + an exit ×, re-centred + re-seeded with the live readouts on relayout.
2. **Activity bar** — a fixed-height slot (`clampf(vh*0.05, 44, 70)` px) below the top bar that telegraphs the treefall, so the board never jumps: an **idle** rail (calm "keep merging" caption) and a **warning** strip (tree icon · "Treefall in Ns" from the 4 s `Explore.WARN` · a draining countdown fill), plus a down-chevron above the doomed column so the eye reads bar → arrow → column. *(One indicator only — a second activity cue is parked.)*
3. **Board** — the 9×7 grid in the shared frame (`Kit.board_panel` / `Kit.slot_cell`), fitted to the band between the activity bar and the bottom hint, centred. (The board no longer fills the whole screen — it took ~80 % empty beige at the start of a run.)
4. **Bottom hint** (`RushBottomHintStrip` / `RushBottomHint`) — "Tap again to fling · empty a column before the treefall", bigger + higher-contrast so it reads as guidance.

The painted backdrop (`board2_bg.png`) is anchored full-rect `KEEP_ASPECT_COVERED`, built once. A guard test (`grove_explore_tests.gd::_test_rush_resize`) drives two viewport widths and asserts each band + the tiles re-fit; `_test_rush_intro_hint` / `_test_screens` guard the node names + build smoke.

**The resident roster (art §9) — 4 merge lines + an ambient flavour layer.** The **merge roster is the four resource lines** (coin-kin · water-kin · boost-kin · diamond-kin), each a 12-step art ladder. The old map-specific kin move to the **ambient-crowd art layer** (wanderers, not merge pieces):

- **Shared ambient core (~3 recolorable wanderers)** — reuse the existing ambient seed-sprites (**moss · acorn · lantern** shapes, §7/§9), recolored per map, on every map.
- **Map-signature ambient specials** — echoing each place's §1 leads + the rotating menagerie: **Farmhouse** hearth-mice / radish-sprites · **Pond** frog-kin · **Orchard** orchard-bees · **Meadow** meadow-flutterers · (later places: lantern-moths, river-otters…). These keep **maps distinct from each other** (variety) even though the production roster is global.
...
**Premium Explore (the v1 gem sink).** Diamonds buy **better box odds / a premium load-out** on the Expedition (odds + speed, never an exclusive spirit — Core §4/§8) — the grove's **recurring gem sink** alongside bag slots + gem variants (§5). *(The earlier "pick a deterministic special on Diamonds" welcome model is superseded by the skill-earned box model; the no-loss surprise-capsule that *is* random stays **post-v1** behind its readiness gate — §1 Scope, Core §4.)*
...
**Invariant reversals (re-proven on `grove_sim`, §5).** The expansion reverses three base assumptions: **capacity** makes the resident sink finite (the old `sink > faucet` proof assumed it was uncapped), **idle production** reopens a passive faucet, and the water/diamond lines mint **Water and Diamonds** — so I2 and the IAP guard must be re-validated, not inherited. Containment is by construction and by **choice**: total Σtier ≤ cells × 12 ≈ 96 **shared across all four lines** (the per-map model ran ~480 concurrently), so maxing one line means zeroing the others; the water line is a small-banked top-up; the diamond line's per-day yield is a designed ceiling behind rare drops; and the free-spirit stream is **cut** so the paid Expedition stays the only spirit source. The re-author — `grove_sim` around the load-out + the four line dials + the rarity table, plus a new **Rush sim** — is the parked §5 economy pass.

### Keeping finished maps alive (anti-abandonment)

*Instantiates Core §8's anti-abandonment.* A completed map must never feel spent. The Residents loop (above) **realizes** the "restored maps stay inhabited" promise — it is the primary anti-abandonment system, with events as the refresh layer on top (a single inhabited world, not two competing ones):

- **Restored maps stay inhabited — the Residents loop *is* this.** Completing a map **grows the shared habitat** (above): the player keeps **bringing home, merging in hand, and placing spirit-folk** for the life of the save, and every completed map keeps its **ambient crowd** (§7) plus its cells' share of the growing bucket. **Future polish (parked):** the placed roster ambient-walks **whichever map the player is currently viewing**, so the one roster always looks at work everywhere. This is the Keepers fiction (§1) made playable: tend the world → its folk come home.
- **Old maps are the live-ops stage — the refresh layer.** Seasonal / limited **events** (Core §17) are **sited in already-restored maps** — a spring bloom fills the Orchard you finished months ago — and bring **ephemeral resident *visitors*** (a time-boxed guest that wanders the habitat during the event, then leaves — never permanently growing the roster), so revisiting is **content, not collection**. The family tend a world that always needs keeping.
...
| **Coins 🪙** | merge drops (~10%) · selling **every tier** (§9) · **quest coins** (§4) · the **coin-kin resident line** (§3) · Shop **5💎→150🪙** | **Expedition load-out boosts** (§3 — the primary, open-ended coin sink) · **generator burst-boost** (§2) · waysides · spirit treats · basket buy-back |
| **Diamonds 💎 (acorns)** | **earned-only & precious** (1 acorn = `COINS_PER_ACORN`=1024🪙): level **milestones** (+3 every 10) · map complete (+5) · the **day-bounded diamond-kin resident line** (§3, the IAP guard) · **piggy crack** (cash) · **cash packs** (live IAP) · **selling a t10–t12 item** (§9, `1 · 2 · 4🌰`) — *never* quests | Water refill (25💎) · Bag slots 7–18 (premium each, Core §5) · gem variants (2–4💎) · **premium Expedition odds** (§3 — better box odds / premium load-out, never an exclusive spirit) · (post-v1) **the surprise-capsule** (§1, Core §4) · **Shop item-shortcuts / exclusive cosmetics** (below) |

### The Merchant (instantiates Core §9)

The **Market Squirrel** runs the stall. The sell ladder is **authored, and DOUBLES per tier from t4** (`SELL_TIER_COINS = [1, 2, 3, 4, 8, 16, 32, 64, 128, …]`) so value tracks the water invested instead of falling behind it — the old linear `round(tier × band)` curve paid a t12 170× less per water click than a t1, and selling was 2% of the coin faucet. **t10–t12 pay ACORNS instead of coins** — flat `1 · 2 · 4🌰`, band-free, no coins (the acorn pinnacle is back, 16× deeper than the retired t8 → 1💎 version: a constant `512💧` per acorn, 12.8× the no-arbitrage floor). **Later-map items sell for more** — a **per-map coin band** (`SELL_MAP_BAND = [1.0, 1.3, 1.7, 2.2, 2.8]`) scales the **coin** tiers up by map (each map an economic step-up); the acorn tiers are flat. See `docs/design/sell-economy-rework.md`; the coin **sinks** are not yet re-priced for this ladder (the parked §5 pass). *(The per-map bands are a grove number — sim-tuned across the arc.)* The grove's buy-back valve is a **wicker basket** at the squirrel's feet (`BASKET_CAP = 3`); a 4th sale overflows and summons the **porter spirit**, who also sweeps the basket every `PORTER_SECS = 180`. Basket never persisted. The Squirrel also keeps the **acorn hoard-jar** — the **piggy-bank vault** (Core §10): it skims a slice of earned 💎, visibly filling, cracked for one fixed real-money price (the §1 commerce-dressed frame).
...
**The primary sink — the Expedition load-out (§3) — gives coins real power**; generator burst (§2) is the second functional sink; the rest are cosmetic / net-zero:

| Sink | Cost | Capacity | Flag | Value |
|---|---|---|---|---|
| **Expedition load-out** | coins per boost item, stacked per run (sim-tuned) | per run — **open-ended** (consumed every Expedition) | — (core loop) | **functional + open-ended** — coins' first unbounded sink; kits out the merge-rush (§3 keystone) |
| **Generator burst boost** | flat 120🪙 per activation (`BOOST_COST`) | per generator, 10 taps then lapses — **repeatable** | — (core loop) | **functional** — better burst odds per tap (cuts taps, not energy; §2) |
...
A plot is dormant (greyed) until its map is fully restored, then shows a coin-cost pin; buying is **coin-only, never level-gated, in no unlock chain**. **Faucet vs sink:** the lifetime coin faucet is merge drops + selling + **quest coins** (Core §7) + the **coin-kin resident line** (§3) — **no hub yield any more** (the old passive faucet is cut, §3); the sinks — the **open-ended Expedition load-out** (§3) + the repeatable generator burst boost (§2) on top of the cosmetic waysides (~1.9 k) — **exceed it** (sink > faucet, *with teeth*), and because the load-out is consumed **per run** there is no finite ceiling for the faucet to catch. *(Capacity upgrades are **no longer a coin sink** — habitat cells come only from map completion, §3: capacity is the faucet's brake, so coins must not buy past it.)* Because coins are now **both sink and faucet**, sink-positivity is re-proven against the **total** in the §5 economy re-author. *(Premium Expedition odds are a separate **diamond** sink, §3 — not coins.)*

> **The "coins have no power" tension — resolved by the v1 Residents expansion (§3, Core §8/§10).** The *legacy* grove sinks (waysides, treats) were all cosmetic / net-zero, so motivation to spend was thin — a decorator pull only. The fix is now **v1**: the **Expedition load-out** (coins kit out the merge-rush — the open-ended sink) plus **generator burst-upgrades** (§2) give coins real power, off the "premium buys speed" line; the cosmetic sinks stay as the **décor layer** on top. And unlike the old model, placed residents now **pay coins back** (the coin-kin line, §3), so coins are both sink and faucet — sink-positivity is re-proven against the total. *(Load-out / production rates are sim-validated in the §5 re-author. The earlier home-hub upgrade→passive-yield loop is **removed** — §1 Scope. The legacy coin-gated bedroom-decor sink — 663🪙 in `districts.gd`/`room.gd` — was retired with the open-space-map model.)*

> **The economy is REOPENED (a fresh balance pass, not a tweak).** Cutting the hub loop deletes both a keystone coin sink (~8,600🪙 ladder) **and** the passive coin faucet (96🪙/day), and the Residents expansion (§3) bolts on new flows — so the §5 numbers need re-authoring, not adjusting. The open work: **(1)** re-author `grove_sim` around the **Expedition load-out** coin sink and the **four resident line dials + the rarity table** (§3 — one bucket, not five per-map streams), plus a new **Rush sim** (fair tide · score→box pacing · no dominant strategy · no forced loss); **(2)** model the **diamond economy** — premium Expedition odds + the **day-bounded diamond-kin line** (the most-scrutinized number, against the IAP ladder) alongside bag slots + gem variants, sized to the gentle cozy-ARPU posture (§1); **(3)** cover the **early-game window** — the bucket only opens once the **first map completes**, so pre-first-completion coins have only burst-upgrades + cosmetics to spend on; verify coins aren't dead there; **(4)** re-derive the **★ scaffold** (the retired 31★/176★ figures, §3) on the unlock-once spots; **(5)** re-prove the reversed invariants (shared-cell opportunity cost bounds the faucet; idle production + Water/Diamonds reopen I2 + the IAP guard; **every merge still pays** — the §3 merge-always-pays invariant, asserted per tier step). All hard invariants stay (no-strand, no-jam, sink > faucet, selling-not-income, I2) — re-validate every dial on `grove_sim` + the Rush sim.
...
| **Rewarded ads** (opt-in, geo-flagged) | refill Water · a doubler on one Expedition run's score / a discounted load-out (§3) · free Shop reroll · event top-up — each **capped + cooldowned**; reward sizes TBD |
...
- **Spirit-folk** are the grove's ambient figures — an **ambient flavour layer**, now decoupled from the production roster (§3 global bucket): each map wanders its own **ambient kin** (a shared core of ~3 recolorable wanderers + map-signature specials, §3), with the crowd growing as the map restores. Tap → `hop`. The **"crowd as it heals"** payoff (§1) comes from that per-place variety plus the **Expedition-and-place loop**. **Future polish (parked, §3):** the **global placed roster** ambient-walks whichever map the player is currently viewing — the one roster always looks at work everywhere. They're the **"restored maps stay inhabited"** life (§3 anti-abandonment) and the build-reveal **crowd** (a §6 incoming flag). *(`ambient_characters` flag §6 — its `1 + restored maps (cap 5)` rule stands for the ambient layer; the old "wanderers are a render of the per-map roster" rule is **retired** with the per-map rosters.)*
