...
- Highest task number so far: **T21** (T21 = map model — one image = one map + map-select + home-hub shortcut, `tasks/mechanics.md`, VERIFIED, commit pending the `zone`→`map` rename call; T20 = grove content roster — v1 lines + generators, `tasks/content-art.md`, done; T19 = §7 generated quests + cutover, done; T18 = retire evolve-merge + the generator-grant hand-in; T17 = per-zone generator engine mechanic — T17–T19 in `tasks/mechanics.md`).
