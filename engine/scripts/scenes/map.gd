...
func _build_map() -> void:
	...
	# ambient life wanders over the map (order L), positioned within the image rect. On a COMPLETED
	# map the wanderers ARE its residents (the §1 population sub-game) — one sprite per roster member,
	# rebuilt from G.resident_members; an in-progress map keeps its baseline generic ambient.
	var amb: Control
