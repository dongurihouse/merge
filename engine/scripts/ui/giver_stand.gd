...
static func make(qi: int, q: Dictionary, cfg: Dictionary) -> Dictionary:
	...
	# A framed quest card (reference layout): a round PORTRAIT in the left oval socket, the
	...
	var isz := (cardH * 0.34) if asks.size() >= 2 else (cardH * 0.44)   # sized to sit INSIDE the item panel
	var span := isz * 0.88
	for ai in asks.size():
