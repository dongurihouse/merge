...
static func build(cfg: Dictionary) -> Dictionary:
	...
	# Sell pill: advertises what the TOP merge tier sells for. Option A retired the t8 premium pinnacle —
	# every tier now sells for banded COINS (no acorns from selling), so the pill reads off TOP_TIER and
	# shows a coin figure. Per the chrome rule the number is pure ASCII ("+N") and the currency is a
	# Look.icon sprite, never an emoji in the text. The figure tracks sell_reward so it can't drift.
	var top_rw := G.sell_reward(100 + G.TOP_TIER)   # top-tier sell -> Vector2i(coins, 0)
