...
## a texture under games/grove/assets that is NOT inside a .gdignore subtree and NOT dropped
## by the iOS preset's exclude_filter. Both selectors must agree, so this reads the real
...
## Every shipped texture's (source_path, compress_mode), skipping .gdignore subtrees.
func _shipped_textures(dir: String, prefixes: Array, out: Array) -> void:
	var d := DirAccess.open(dir)
	if d == null:
		return
	if d.file_exists(dir + "/.gdignore"):
		return
	d.list_dir_begin()
	var name := d.get_next()
	while name != "":
...
func _initialize() -> void:
	...
	var imports: Array = []
	_shipped_textures(ASSETS, prefixes, imports)
	...
	for imp in imports:
		if _mode_of(imp) == 0:                 # 0 == Lossless; 1 == lossy WebP; 2 == VRAM
			...
			if not LOSSLESS_ALLOWLIST.has(src):
				lossless.append(src.replace("res://games/grove/assets/", ""))
	ok(lossless.is_empty(),
		"no shipped texture is lossless%s" % ("" if lossless.is_empty()
