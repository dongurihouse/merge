...
## A fixture roster (independent of the live grove data): map 0 grants 2 generators
## outright; map 1 brings 3 — two are HAND-IN grants of map 0's pair, one is a surplus.
func _fixture() -> Array:
	return [
		{"id": "g0a", "map": 0, "lines": [1, 2], "grant_from": "", "cell": Vector2i(4, 3)},
		{"id": "g0b", "map": 0, "lines": [3, 4], "grant_from": "", "cell": Vector2i(2, 1)},
		{"id": "g1a", "map": 1, "lines": [5, 6], "grant_from": "g0a"},
		{"id": "g1b", "map": 1, "lines": [7, 8], "grant_from": "g0b"},
		{"id": "g1c", "map": 1, "lines": [10, 11], "grant_from": "", "cell": Vector2i(6, 5)},
	]
...
func _initialize() -> void:
	...
	# --- the authored generator-grant quests that open a map (§6/§7 trigger) ---
	var gq := G.grant_quests_for_map(r, 1)
	ok(gq.size() == 2, "map 1 authors 2 grant quests — one per hand-in generator; the surplus is outright")
	...
	ok(granted_ids.has("g1a") and granted_ids.has("g1b"), "the grant quests dispense the map's two hand-in generators")
	ok(String(gq[0].grant.hand_in) == "g0a", "the first grant quest hands in g1a's predecessor g0a")
	ok(G.grant_quests_for_map(r, 0).is_empty(), "map 0 authors no grant quests (its set is granted outright)")
	...
	# --- cell resolution + the live set per map (interim "grant on map entry") ---
	...
	ok(s1.size() == 3 and s1[Vector2i(4, 3)] == "g1a" and s1[Vector2i(2, 1)] == "g1b" and s1[Vector2i(6, 5)] == "g1c", "map 1 live set: 2 grants (inherited cells) + 1 surplus")
