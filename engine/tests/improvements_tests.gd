...
func _test_seed_rules_drop_filter_and_eligibility() -> void:
	...
	ok(Improvements.seed_sell_reward(Improvements.KIND_SOIL) == Vector2i(520, 0), "soil seed sells for 520 coins")
	ok(Improvements.seed_sell_reward(Improvements.KIND_MAGNET) == Vector2i(520, 0), "magnet seed sells for 520 coins")
	...
	ok(Improvements.soil_rank_price(1) < 0 and Improvements.soil_rank_price(2) < 0, "soil rank is disabled, so no rank price is available")
	ok(Improvements.soil_step_seconds(101, 1) == 10.0, "soil tier-1 step takes 10 seconds")
	ok(Improvements.soil_step_seconds(107, 2) == 14400.0, "soil rank is disabled, so higher raw ranks apply no time discount")
...
func _test_board_model_persists_improvements() -> void:
	...
	ok(String(got.kind) == Improvements.KIND_SOIL and int(got.rank) == 1 and int(got.code) == 104 and bool(got.watered), "old ranked improvement rows load as rank-1 Soil")
	ok(b2.item_at(seed_cell) == Improvements.seed_code_for_kind(Improvements.KIND_SOIL) and b2.seed_rank_at(seed_cell) == 1 and b2.seed_ranks.is_empty(), "old ranked seed metadata is ignored")
	var blob := b.to_dict()
...
func _test_soil_reconcile_water_and_completion() -> void:
	...
	ok(b.item_at(r3) == 107, "soil rank is disabled, so a raw rank-3 row still grows one tier per completion")
...
func _test_place_seed_unsocket_and_generator_skip() -> void:
	...
	ok(b.item_at(cell) == 0 and String(placed.kind) == Improvements.KIND_SOIL and int(placed.rank) == 1 and Save.coins() == coins_b, "seed placement is free and ignores stale Soil rank metadata")
	b.place(cell, 101)
	...
	ok(not b.has_improvement(cell) and b.item_at(cell) == Improvements.seed_code_for_kind(Improvements.KIND_SOIL) and b.seed_rank_at(cell) == 1, "unsocket leaves a plain Soil seed in the same cell")
	ok(BoardActions.place_seed(b, cell).placed and int(b.improvement_at(cell).rank) == 1, "a returned Soil seed places again at rank 1")
