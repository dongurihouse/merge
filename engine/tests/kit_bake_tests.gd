...
func _initialize() -> void:
	...
	ok(srcs.has(Game.art("ui/meadow_v2/icon_settings.png")), "the Meadow Settings icon is in the bake set")
	ok(srcs.has(Game.art("ui/meadow_v2/icon_mail.png")), "the Meadow Mail icon is in the bake set")
	# 4c. The Play CTA + live-ops rail icons that landed AFTER the first chrome pass: the board/vine play
	#     marks and the calendar/chest rail icons. They were the live-polish cost in _build_chrome /
	#     _build_liveops_rail; _chrome() builds a home_button for each so they land in the bake set
	#     (and the GUARD above holds each one baked).
	for icon_id in ["board", "vine"]:
		ok(srcs.has(Game.art("ui/shared/icon_%s.png" % icon_id)), "the %s chrome icon is in the bake set" % icon_id)
	ok(srcs.has(Game.art("ui/meadow_v2/icon_daily.png")), "the Meadow Daily icon is in the bake set")
	ok(srcs.has(Game.art("ui/meadow_v2/icon_vault.png")), "the Meadow Vault icon is in the bake set")
	for n in nodes:
		if n is Node:
