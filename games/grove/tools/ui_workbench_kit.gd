...
static func dialog_frame(content: Control, width: float = 560.0, opts: Dictionary = {}) -> Control:
	...
	card.custom_minimum_size = Vector2(width, maxf(0.0, min_h_override))   # height floor finalised in relayout (needs the viewport for the %)
	...
	var relayout := func() -> void:
		...
		if card.is_inside_tree():
			...
			if center_content and is_instance_valid(content):
				...
				if absf(content.custom_minimum_size.y - fill) > 1.0:
					content.custom_minimum_size.y = fill
