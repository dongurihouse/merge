...
## with its always-on bottom hint and optional in-scene hand teaches.
##   quiet_godot.sh --path . -s res://games/grove/tools/rush_shot.gd -- <mode> <out.png> [WxH]
## modes: rush (default — bottom hint) | treefall (telegraphed treefall at ~4s) |
##        merge_hint (the FTUE hand teach on a seeded mergeable pair)
...
func _initialize() -> void:
	...
	root.add_child(scn)
	current_scene = scn
	await create_timer(0.12).timeout               # let _ready build the board
	if mode == "merge_hint":                        # a deterministic board with a guaranteed mergeable pair
		...
		for _i in 6:                                # seed a few tiles so the board reads as a live rush
			scn._spawn()
	await create_timer(0.4).timeout                 # tiles settle before capture
