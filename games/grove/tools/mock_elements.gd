...
# --- the SHOP -----------------------------------------------------------------------------
# NO ADAPTER. The storefront had one (`shop_card`, the sage offer card on the v3 mock's cream sheet); the
# card was retired on 2026-07-30 when the shop became the MARKET STALL, so there is no element left for it
# to build. The stall itself cannot be rigged here at all: every one of its parts sits on the stall's own
# painted wall or on a shelf plank, and that mock has no flat ground — the numbers are recorded on the
# `shop_market_stall` entry in mock_targets.json, which carries `field_candidates` instead of regions and
# is re-measured on every `make test-config`. The stall's MATERIAL is still covered, on meadow_board,
# through the shared cut-paper panel every one of its sheets is drawn by.
