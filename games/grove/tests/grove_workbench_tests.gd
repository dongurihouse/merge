...
func _is_reference_paper_shadow(style: StyleBoxFlat) -> bool:
	# THE uniform shadow = whatever the SAVED workbench block says (behind-panel: size is the blur,
	# expand margin is the spread) — the contract is "follows the saved block", not one fixed look.
	var p := Look.saved_shadow_params()
	return style != null \
		and _same_rgb(style.shadow_color, Color(0, 0, 0)) \
		and absf(style.shadow_color.a - float(p.alpha)) <= 0.01 \
		and style.shadow_offset.is_equal_approx(Vector2(float(p.offset_x), float(p.offset_y))) \
		and style.shadow_size == int(float(p.blur)) \
		and absf(style.get_expand_margin(SIDE_BOTTOM) - float(p.spread)) <= 0.01
...
func _initialize() -> void:
	...
	# SETTINGS is NOT a bar tile: it is a plain gear pinned top-right, one shared margin below the
	# wallet pills — no paper face, no caption, no shadow.
	ok(map_scene._gear != null and map_scene._gear.name == "SettingsGear" \
		and String((map_scene._gear as Button).tooltip_text) == "Settings" \
		and map_scene._chrome_nodes.has(map_scene._gear), \
		"Settings is a plain gear tracked as chrome, not a bottom-bar tile")
	ok(_paper_texture_path(map_scene._gear) == "", \
		"the Settings gear wears no paper face — the bare icon is the button")
	ok(map_scene.get_node_or_null("SettingsTile") == null, \
		"Settings no longer occupies a slot in the bottom bar")
	if map_scene._gear != null:
		var gear_rect := (map_scene._gear as Control).get_global_rect()
		ok(absf(map_screen_w - gear_rect.end.x - edge_margin) <= 1.0, \
			"the gear sits at the shared margin from the right edge")
		if map_scene._hud_panels.size() > 0:
			var wallet_ctl := map_scene._hud_panels[0] as Control
			var below: float = gear_rect.position.y - (wallet_ctl.get_child(0) as Control).get_global_rect().end.y
...
func _test_new_knobs(view) -> void:
	...
	# the level sheet is built on the SHARED frame, UNMODIFIED — the standard cream card + the frame's
	# navy title band ("LEVEL 9") + the standard ✕ disc, not a bespoke card.
	...
	ok(level_preview.find_child("DialogClose", true, false) != null, \
		"the level dialog carries the shared frame's ✕ disc (standard frame, unmodified)")
...
func _is_neutral_shadow(color: Color) -> bool:
	return color.a > 0.0 and absf(color.r - color.b) < 0.02 and absf(color.r - color.g) < 0.02
...
func _test_warm_shadow_port() -> void:
	...
	for sh in [Look.shadow_rect(40.0, p), Look.shadow_circle(140.0, p)]:
		...
		ok(st != null and _same_rgb(st.shadow_color, Color(0, 0, 0)) and st.shadow_size == 6, \
			"the shared shadow uses the neutral reference shadow tint")
	...
	for pan in board.find_children("*", "Panel", true, false):
		...
		if sb != null and sb.shadow_size > 0:
			board_meadow_shadow = _same_rgb(sb.shadow_color, Color(0, 0, 0)) and absf(sb.shadow_color.a - float(p.alpha)) <= 0.01
			break
	...
	ok(home_style != null and _same_rgb(home_style.shadow_color, Color(0, 0, 0)) and absf(home_style.shadow_color.a - float(Look.shadow_params({}).alpha)) <= 0.01,
	...
	ok(absf(float(wallet_opts.shadow_params.alpha) - float(Look.shadow_params({}).alpha)) <= 0.01,
		"wallet shadow resolver ignores per-component overrides — the shared block (defaults here) only")
	...
	ok(_is_neutral_shadow(baked.get_pixel(18, 18)), \
		"baked icon/badge shadows use the neutral reference shadow tint")
...
func _test_quest_card_config(view) -> void:
	...
	for p in con.find_children("*", "Panel", true, false):
		...
		if sb is StyleBoxFlat and (sb as StyleBoxFlat).shadow_size > 0 \
			and _same_rgb((sb as StyleBoxFlat).shadow_color, Color(0, 0, 0)) \
			and absf((sb as StyleBoxFlat).shadow_color.a - float(on_params.alpha)) <= 0.01:
			slate_shadow = true
