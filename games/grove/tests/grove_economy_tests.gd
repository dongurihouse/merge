...
func _test_market_cluster_helpers() -> void:
	...
	var unlocks := {}
	ok(G.next_locked_cluster(0, unlocks) == "mushroom_hall", "market: top cluster is first in the sequence")
	ok(G.cluster_locked(0, "tea_stall", unlocks), "market: tea_stall starts locked")
	ok(not G.cluster_ready(0, "mushroom_hall", unlocks, 0, 0), "market: under-leveled/too-poor is not ready")
	ok(G.cluster_ready(0, "mushroom_hall", unlocks, 1, 10), "market: gate met -> ready")
	ok(not G.cluster_ready(0, "tea_stall", unlocks, 9, 9999), "market: a later cluster is never ready under strict sequence")
	unlocks["mushroom_hall"] = true
	ok(G.next_locked_cluster(0, unlocks) == "tea_stall", "market: the sequence advances after an unlock")
	ok(G.cluster_ready(0, "tea_stall", unlocks, 9, 9999), "market: the next cluster becomes ready")
