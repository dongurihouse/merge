...
grep -q 'const MARKETING_VERSION := "1.0"' "$OUT" || fail "bare export did not use export preset marketing version"
grep -q 'const BUILD_NUMBER := "77"' "$OUT" || fail "bare CI export did not use CI build number"

"$SCRIPT" "$OUT"
grep -q 'const MARKETING_VERSION := "1.0"' "$OUT" || fail "bare local export did not use export preset marketing version"
grep -q 'const BUILD_NUMBER := "1"' "$OUT" || fail "bare local export did not use export preset build number"
