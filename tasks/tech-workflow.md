...
### T15 — Consolidated spec doc (`merge_spec`) · 2026-06-14 · workflow · done
- **Asked:** "review all docs, create a single doc with the spec and the feature set, call it merge_spec."
- **Problem:** the design truth was spread across ~18 docs (spec, milestones, features, friction, UI, art, audio) + ledgers, several of them stale vs the code; no single authoritative spec.
- **Type:** new (process/docs)
- **LLM-reliability:** med — synthesis from sources is mostly deterministic, BUT completeness + catching stale-doc-vs-code drift is the risk; mitigated by code-verifying every number.
- **Human-in-loop:** recommended — the owner is the authority on what belongs in the canonical spec; a read-through is the close.
- **Verification:** built via a 6-agent doc/code extraction workflow (concept · mechanics · world/economy · features · art/audio · tech), each told CODE WINS over docs; folded a parallel coin-economy audit into §7/§16. Cross-checked board 7×9, the 24 feature flags vs features.gd, currency numbers vs grove_content/save.
- **Iterations:** 1
- **Result:** `docs/merge_spec.md` (now **17 sections** — added §2 Progression: Chapters vs Levels per owner Q; folded the **feature index** `FEATURES.md` into §9 with *Lives in* + owner *Eval* columns + core/tuning/removed-history, and stubbed `FEATURES.md` to a redirect). Surfaced: T14 flags aren't on `main` (branch only); the only coin-progression-gate sink is **legacy** districts code, not the live grove (§17 flags the "coins have no power" tension); `main` has **25** flags (the extraction agent miscounted 24 — fixed). Not committed (owner to review).
